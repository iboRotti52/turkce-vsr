...
>
> Large-data deneyinde bilimsel soruyu seçtikten sonra normal agent giriş noktası olarak `python -m src.experiments.research_ops` kullan. Önce `status` ile candidate + plan + registry + round/belief belleğini uzlaştır. Smoke/10h/25h/50h/100h/full-data scale'lerini yalnız gerçek `large_data_plan.json` içindeki mevcut stage'lerden kullan. Daha küçük scale atlanıyorsa nedenini yaz. Deneyden **önce** hypothesis, falsification criteria, expectation, information-gain rationale, promotion rule, estimated GPU-hours/USD ve evidence scope'u spec'e yazıp `research_ops register` ile registry'ye `IN_PROGRESS` olarak mühürle. Düşük seviyeli `large_data_controller` API'sini normal agent akışında elle orkestre etme.
>
